...
##### Usage
The `<script>` tag exposes the `languageEncoding` function to everything in the DOM located beneath it. You should have no trouble accessing it with Javascript by calling the languageEncoding function and passing in the file that you want to analyze as the argument. As you can see in the example below, languageEncoding returns a Promise that you can use to get the encoding, language and confidenc score. 
